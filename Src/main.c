...
void TestBrake(void) { 
	unsigned int keyb;
	squareXY(0, 0, 127, 63, 0);
	putstr(3, 0, "Brake test", 0);
	putstr(0, 2, "Press KEY UP to", 0);
	putstr(4, 3, "start", 0);
	putstr(1, 5, "Press KEY DOWN", 0);
	putstr(3, 6, "to finish", 0);
	update_display();
	
	while ((keyb = kbdread()) == 0) continue;
	switch (keyb) {
		case KEY_UP:
			delay_us(1000000);
			BrakeTest();
